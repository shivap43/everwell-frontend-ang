...
import { MemberInfoState, TPIState, AccountListState } from "@empowered/ngxs-store";
...
export class ManageDependentComponent implements OnInit, AfterViewChecked, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.mpGroupId = this.isTPI
            ? this.store.selectSnapshot(TPIState.tpiSsoDetail).user.groupId
            : this.store.selectSnapshot(AccountListState.getMpGroupId);
        this.MemberInfo = this.store.selectSnapshot(MemberInfoState.getMemberInfo);
        this.memberId = this.isTPI ? this.store.selectSnapshot(TPIState.tpiSsoDetail).user.memberId : this.MemberInfo.id;
        ...
        this.subscriptions.push(
            this.memberService.getMemberDependents(this.memberId, false, this.mpGroupId).subscribe(
                (dependents) => {
