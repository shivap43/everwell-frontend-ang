...
import { DeclineCoverageComponent } from "./decline-coverage/decline-coverage.component";
import { Permission, AppSettings, TaxStatus, PolicyOwnershipType, TobaccoStatus } from "@empowered/constants";
...
export class EditCoverageComponent implements OnInit, OnDestroy {
    ...
    languageStrings: Record<string, string> = this.language.fetchPrimaryLanguageValues([
        ...
        "primary.portal.editCoverage.declinecoverage",
    ]);
    isDisable = false;
    ...
    portal: any;
    endDateEditConfig = "group.employee.coverage.end_date.edit";
    editCoverageTitle: string;
    ...
    ngOnInit(): void {
        ...
        if (this.data.enrollData) {
            ...
            this.editCoverageTitle = `${this.languageStrings["primary.portal.editCoverage.edit"]}  ${this.data.enrollData.plan.name} ${this.languageStrings["primary.portal.editCoverage.coverage"]}`;
        }
    }
    ...
    decidePlanStatus(): void {
        ...
        } else if (this.data.enrollData.validity.expiresAfter != null) {
            this.coverageForm.get("coverageEndDate").setValue(this.data.enrollData.validity.expiresAfter);
            this.staticUtilService.cacheConfigEnabled(this.endDateEditConfig).subscribe((enabled) => {
                if (
                    enabled &&
    ...
    voidCoverage(planName: number): void {
        const voidCoverageDialog = this.empoweredModalService.openDialog(VoidCoverageComponent, {
            data: {
                planName: planName,
    ...
    checkForChanges(type?: any): void {
        ...
        this.isDataChange = this.changeFields.length > 0;
    }
    ...
    setEnrollData(data: any): void {
        ...
        this.isLoading = true;
        this.isData.Current = data.status === EnrollmentStatusType.APPROVED;
    ...
    declineModal(): void {
        ...
        this.subscription.add(
            this.shoppingService
                .getPlanOffering(this.data.enrollData.planOfferingId, this.data.mpGroup, "plan.productId")
                .subscribe((plan) => {
                    this.isLoading = false;
                    const declineCoverageDialog = this.dialog.open(DeclineCoverageComponent, {
                        width: "40rem",
                        data: {
                            mpGroup: this.data.mpGroup,
                            memberId: this.data.memberId,
                            ...
                            productName: plan.plan.product.name,
                            planYear: plan.planYearId ? plan.planYearId : undefined,
                            productOfferingId: plan.productOfferingId,
                            enrollId: this.data.enrollData.id,
                        },
                    });
                    declineCoverageDialog.afterClosed().subscribe((data) => {
                        if (data !== undefined) {
                            this.onCancelClick(data);
                        }
                    });
